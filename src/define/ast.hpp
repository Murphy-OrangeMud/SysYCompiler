...
namespace SysYToEeyore {
    ...
    public:
        ProcessedIdAST(std::string _name, VarType _type, bool _const = false,
                       std::vector<int> _dim = std::vector<int>{}) :
                Const(_const), name(std::move(_name)), type(_type), dim(std::move(_dim)) {}
    ...
    private:
        ...
        std::vector<int> dims;
    public:
        InitValAST(VarType _type, ASTPtrList list, std::vector<int> _dims = std::vector<int>{}) : type(_type),
                                                                                                  values(std::move(
                                                                                                          list)),
                                                                                                  dims(_dims) {}
