...
namespace EeyoreToTigger {
    ...
    void LiveAnalysis::AnalyzeStatements(StatementsIR &stmts, std::set<std::string> &nextVars) {
        ...
        for (int i = stmts.getStmts().size() - 1; i >= 0; i--) {
            ...
            if (dynamic_cast<GotoIR *>(stmts.getStmts()[i].get())) {
                label_goto[dynamic_cast<GotoIR *>(stmts.getStmts()[i].get())->getLabel()].second = std::max(i + 1,
                                                                                                            label_goto[dynamic_cast<GotoIR *>(stmts.getStmts()[i].get())->getLabel()].second);
            }
            if (dynamic_cast<CondGotoIR *>(stmts.getStmts()[i].get())) {
                label_goto[dynamic_cast<CondGotoIR *>(stmts.getStmts()[i].get())->getLabel()].second = std::max(i + 1,
                                                                                                                label_goto[dynamic_cast<CondGotoIR *>(stmts.getStmts()[i].get())->getLabel()].second);
            }
        }
        for (int i = stmts.getStmts().size() - 1; i >= 0; i--) {
            ...
            if (dynamic_cast<Label *>(stmts.getStmts()[i].get())) {
                ...
                while (!flag) {
                    ...
                    for (int j = range.second; j >= range.first; j--) {
                        ...
                        std::set_difference(tmp.begin(), tmp.end(), stmts.getStmts()[j]->liveVars.begin(),
                                            stmts.getStmts()[j]->liveVars.end(), diff.begin());
                        if (diff.size() > 0) {
                            flag = false;
                        }
                    }
            ...
            if (i < stmts.getStmts().size() - 1) {
                ...
                stmts.getStmts()[i]->Analyze(*this, tmp);
                logger.UnSetFunc("AnalyzeStatements");
            }
        }
    }
    ...
    void LiveAnalysis::AnalyzeAssign(AssignIR &assign, std::set<std::string> &nextVars) {
        ...
        std::set<std::string> tmp;
        std::set_difference(assign.liveVars.begin(), assign.liveVars.end(), assign.getLHS()->liveVars.begin(),
                            assign.getLHS()->liveVars.end(), tmp.begin());
        assign.liveVars = tmp;
    }
    ...
    void LiveAnalysis::AnalyzeProgram(ProgramIR &program, std::set<std::string> &nextVars) {
        ...
        for (int i = program.getNodes().size() - 1; i >= 0; i--) {
            program.getNodes()[i]->Analyze(*this, tmp);
